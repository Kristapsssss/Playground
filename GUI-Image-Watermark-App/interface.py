...
button_open = customtkinter.CTkButton(image_frame, width=50, height=50, text='Open Image From File Directory', command=lambda: app.open_image(label_image), font=('arial', 20, 'bold'))
button_save = customtkinter.CTkButton(image_frame, width=50, height=50, text='Save', command=lambda: app.save_image(canvas=label_image), font=('arial', 20), fg_color=('darkgreen', 'darkgreen'))
...
button_watermark_open = customtkinter.CTkButton(control_frame, width=50, height=50, text='Select Watermark Image', command=app.draw_watermark, font=('arial', 20, 'bold'))
button_delete_watermark = customtkinter.CTkButton(control_frame, width=50, height=50, text='Remove Watermark', command=app.delete_watermark, font=('arial', 15), fg_color=('darkred', 'darkred'))

label_watermark_text = tk.Label(control_frame, text='Watermark Text:', bg="powderblue", font=('arial', 13))
watermark_text = customtkinter.CTkEntry(control_frame)

button_add_text = customtkinter.CTkButton(control_frame, width=50, height=50, text='Add', command=lambda: [app.draw_text(watermark_text), app.get_font(combobox_font), app.get_color(combobox_color)], font=('arial', 15))
button_remove_text = customtkinter.CTkButton(control_frame, width=50, height=50, text='Remove', command=app.remove_text, font=('arial', 15), fg_color=('darkred', 'darkred'))

label_font = tk.Label(control_frame, text="Select Text Font:", bg="powderblue", font=('arial', 13))
combobox_font = customtkinter.CTkComboBox(control_frame, values=["arial", "arialbd", "BASKVILL", "BRADHITC", "cambriab", "FREESCPT", "ITCBLKAD", "unispace bd", "verdana", "TEMPSITC"])

label_font_color = tk.Label(control_frame, text="Select Text Color:", bg="powderblue", font=('arial', 13))
combobox_color = customtkinter.CTkComboBox(control_frame, values=["antiquewhite", 'black', 'tomato', 'pink', 'purple', 'wheat', 'springgreen', 'powderblue', 'blanchedalmond', 'hotpink', 'magenta'])
...
combobox_font.grid(row=2, column=1, padx=10, pady=10)

label_font_color.grid(row=3, column=0, pady=5)
combobox_color.grid(row=3, column=1, pady=10)
...
button_exit = customtkinter.CTkButton(remote_frame, width=50, height=50, text='Exit', command=root.destroy, font=('arial', 20), fg_color=('darkred', 'darkred'))
